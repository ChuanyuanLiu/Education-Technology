...
function FrameworkOverview({history}) {
    ...
    const addSection = () => {
        const url = `http://139.99.155.172:3001/framework/section/new?framework_id=${framework_id}`;
        fetch(url)
    ...
    const addQuestion = (section_id) => {
        const url = `http://139.99.155.172:3001/framework/section/question/new?section_id=${section_id}`;
        fetch(url)
            .then((response) => response.json())
            .then((data) => {
                setSections((prevState) =>
    ...
    const setActive = () => {
        const url = `http://139.99.155.172:3001/framework/activestatus/update?framework_id=${framework_data.framework_id}`;
    ...
    const handleFinalize = () => {
        const url = `http://139.99.155.172:3001/framework/finalisedstatus/update?framework_id=${framework_data.framework_id}`;
    ...
    const handleNewVersion = () => {
        const url = `http://139.99.155.172:3001/framework/version?framework_id=${framework_id}`;
        fetch(url)
            .then((response) => response.json())
            .then((data) => {
                // history.replace({
    ...
    const postTitle = (framework_id) => (text) => {
        const url = `http://139.99.155.172:3001/framework/update?framework_id=${framework_id}`;
        const param = {
            headers: {"Content-Type": "application/json"},
    ...
    useEffect(() => {
        ...
        fetch(`http://139.99.155.172:3001/framework?framework_id=${framework_id}`)
            .then((response) => response.json())
            .then((data) => {
                if(!isCancelled){
...
function EditableSection(props) {
    ...
    const toggleSave = (event) => {
        event.preventDefault();
        setActive(!getActive);
        const url = `http://139.99.155.172:3001/framework/section/update?section_id=${props.section.section_id}`;
        const param = {
            headers: {"Content-Type": "application/json"},
