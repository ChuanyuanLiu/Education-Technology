...
describe("AC 2.2.9", () => {
    ...
    test("Correct fetch request", async ()=> {
        ...
        expect(fetch.mock.calls[0][0]).toBe(`https://localhost:3001/framework?question_id=${QUESTION_ID}`);
    });
    ...
    test("Edit ratings", async() => {
        ...
        expect(fetch.mock.calls[0][0]).toBe(`https://localhost:3001/framework/section/question/rate/update?question_id=${QUESTION_ID}`)
        // Only called once
    ...
    test("Edit question title", async()=> {
        ...
        expect(fetch.mock.calls[0][0]).toBe(`https://localhost:3001/framework/section/question/update?question_id=${QUESTION_ID}`)
        // Only called once
        expect(fetch.mock.calls.length).toBe(1);
    })
});
