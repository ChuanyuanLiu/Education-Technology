...
function UserOverview({history}) {
    ...
    const updateText = (field, isMetadata) => (text) => {
        ...
        fetch(`https://localhost:3001/user/update?user_id=${user_id}`, param)
            .then((response) => {
    ...
    const handleRoleChange = (value) => {
        ...
        fetch(`https://localhost:3001/user/update/role?user_id=${user_id}`, param)
            .then((response) => {
    ...
    const deleteUser = () => {
        ...
        fetch(`https://localhost:3001/user/delete?user_id=${user_id}`, param)
            .then((response) => {
                if (response.status === 200) {
                    console.log("success");
                    history.goBack();
    ...
    useEffect(() => {
        let isInit = false;
        fetch(`https://localhost:3001/user?user_id=${user_id}`)
            ...
            .catch(console.error);
        fetch("https://localhost:3001/user/roles")
            .then((response) => response.json())
