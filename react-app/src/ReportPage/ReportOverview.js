...
function ReportOverview ({history}){
    ...
    const post_title_url = `https://${process.env.REACT_APP_DOMAIN}:3001/report/update/title?report_id=${report_id}`
    const post_summary_url = `https://${process.env.REACT_APP_DOMAIN}:3001/report/update/recommendation?report_id=${report_id}`
    const post_finailized_url = `https://${process.env.REACT_APP_DOMAIN}:3001/report/finalise?report_id=${report_id}`
    const download_url = `https://${process.env.REACT_APP_DOMAIN}:3001/report/download?report_id=${report_id}`
    useEffect(() => 
        {
            let isCancelled = false;
            fetch(`https://${process.env.REACT_APP_DOMAIN}:3001/report?report_id=${report_id}`)
