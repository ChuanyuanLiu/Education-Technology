...
function ReportOverview ({history}){
    ...
    const post_title_url = `https://localhost:3001/report/update/title?report_id=${report_id}`
    const post_summary_url = `https://localhost:3001/report/update/recommendation?report_id=${report_id}`
    const post_finailized_url = `https://localhost:3001/report/finalise?report_id=${report_id}`
    const download_url = `https://localhost:3001/report/download?report_id=${report_id}`
    useEffect(() => 
        {
            let isCancelled = false;
            fetch(`https://localhost:3001/report?report_id=${report_id}`)
